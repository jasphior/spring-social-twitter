...
/**
 * Implementation of {@link TimelineOperations}, providing a binding to
 * Twitter's tweet and timeline-oriented REST resources.
 * 
 * @author Craig Walls
 */
class TimelineTemplate extends AbstractTwitterOperations implements TimelineOperations {
	...
	public List<Tweet> getHomeTimeline(int page, int pageSize, long sinceId, long maxId) {
		...
		MultiValueMap<String, String> parameters = PagingUtils.buildPagingParametersWithCount(page, pageSize, sinceId,
		        maxId);
		parameters.add("include_my_retweet", "true");
	...
	public List<Tweet> getRetweets(long tweetId, int count) {
		...
		return restTemplate.getForObject(buildUri("statuses/retweets/" + tweetId + ".json", parameters),
		        TweetList.class);
	}
	...
	public List<TwitterProfile> getRetweetedBy(long tweetId, int page, int pageSize) {
		...
		return restTemplate.getForObject(buildUri("statuses/" + tweetId + "/retweeted_by.json", parameters),
	...
	public List<Long> getRetweetedByIds(long tweetId, int page, int pageSize) {
		requireAuthorization(); // requires authentication, even though
		                        // getRetweetedBy() does not.
		MultiValueMap<String, String> parameters = PagingUtils.buildPagingParametersWithCount(page, pageSize, 0, 0);
		return restTemplate.getForObject(buildUri("statuses/" + tweetId + "/retweeted_by/ids.json", parameters),
		        LongList.class);
	}
	...
	public List<Tweet> getFavorites(int page, int pageSize) {
		requireAuthorization();
		// Note: The documentation for /favorites.json doesn't list the count
		// parameter, but it works anyway.
		MultiValueMap<String, String> parameters = PagingUtils.buildPagingParametersWithCount(page, pageSize, 0, 0);
		return restTemplate.getForObject(buildUri("favorites.json", parameters), TweetList.class);
	}

	public Tweet addToFavorites(long tweetId) {
		requireAuthorization();
		MultiValueMap<String, Object> data = new LinkedMultiValueMap<String, Object>();
		return restTemplate.postForObject(buildUri("favorites/create/" + tweetId + ".json"), data, Tweet.class);
	}
